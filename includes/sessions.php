...
function select_session_url($session_page, $url_qs, $url_ps, $specific, $level, $id, $forum_data, $topic_data, $user_data, $cat_data)
{
    ...
    global $HTTP_SERVER_VARS;
        ...
        if (isset($HTTP_SERVER_VARS['QUERY_STRING']))
            $qs = preg_replace('/%09/i', '%20', $HTTP_SERVER_VARS['QUERY_STRING']);
        elseif (getenv('QUERY_STRING'))
            $qs = preg_replace('/%09/i', '%20', getenv('QUERY_STRING'));
        else
            $qs = 'unknown';
...
function append_sid($url, $non_html_amp = false)
{
    ...
    if (strstr($url, "admin=1") || strstr($url, "admin_") || strstr($url, "pane=")){
                                //  The format is fine, don't change a thing.
    } else if (strstr($url, "Your_Account")){
    ...
    else if ((strstr($url, "privmsg")) && (!strstr($url, "highlight=privmsg"))) {
            $url = ($non_html_amp) ? str_replace("?", "&", $url) : str_replace("?", "&amp;", $url); // As we are already in nuke, change the ? to &
 ...
 ******************************************************/
    else if ((strstr($url, "groupcp")) && (!strstr($url, "highlight=groupc"))) {
            $url = ($non_html_amp) ? str_replace("?", "&", $url) : str_replace("?", "&amp;", $url); // As we are already in nuke, change the ? to &
 ...
 ******************************************************/
    else if ((strstr($url, "profile")) && (!strstr($url, "highlight=profile"))) {
            $url = ($non_html_amp) ? str_replace("?", "&", $url) : str_replace("?", "&amp;", $url); // As we are already in nuke, change the ? to &
...
/*****[END]********************************************
 [ Mod:   Profile as own module                v1.0.0 ]
 ******************************************************/
    else if ((strstr($url, "memberlist")) && (!strstr($url, "highlight=memberlist"))) {
            $url = ($non_html_amp) ? str_replace("?", "&", $url) : str_replace("?", "&amp;", $url); // As we are already in nuke, change the ? to &
