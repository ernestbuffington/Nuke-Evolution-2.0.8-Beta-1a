...
function validate_data($post){
    ...
    if (!(@mysql_select_db($dbname))){
        $error .= '<font color="red">'.$install_lang['connection_failed2'].'</font><br />';
    }
...
function do_sql($install_file){
    ...
    while ((!feof($fp) || strlen($buffer))){
        ...
        if ($inside_quote && $current_char == $quote_inside && $last_char != '\\'){
            ...
            if ($errors && mysql_errno()){
                $message .= '<font color="red">' . $install_lang['sql_error'] . mysql_errno().': '.mysql_error().'<br />'.$data_buffer.'<br />';
            }
            $data_buffer = '';
            $last_char = "\n";
            $started_query = 0;
    ...
    if (empty($message)){
        $message = '<font color="green">'.$install_lang['install_success'].'</font><br /><br /><input type="hidden" name="step" value="'.$next_step.'" /><input type="submit" class="button" value="'.$install_lang['continue'].' '.$next_step.'" />';
    } else {
...
function validate_admin(){
	global $install_lang, $next_step, $step;
	...
	if ($_POST['admin_pass'] != $_POST['admin_cpass']){
		...
		if (!mysql_query("INSERT INTO " . $_SESSION['prefix'] . "_authors VALUES ('".$_POST['admin_nick']."', 'God', '".$_POST['admin_website']."', '".$_POST['admin_email']."', '".md5($_POST['admin_pass'])."', '0', '1', '')")){
			$error = true;
 ...
 ******************************************************/
		if (!mysql_query("INSERT INTO `" . $_SESSION['prefix'] . "_nsnst_admins` (`aid`, `login`, `protected`) VALUES ('".$_POST['admin_nick']."', '".$_POST['admin_nick']."', '1')")){
			$error = true;
 ...
 ******************************************************/
		$cookiedata = base64_encode("$user_nick:$user_pass:english:1");
		setcookie('admin',$cookiedata,time()+2592000,$cookie_location);
/******************************************************
 [ Mod:    Auto Admin Login                    v2.0.0 ]
 ******************************************************/
/*****[BEGIN]******************************************
...
function site_form($display=1,$return=false){
    ...
    if (!$result = mysql_query($sql)){
        die($install_lang['get_config_error'].mysql_error());
    }
    ...
    while($row = @mysql_fetch_assoc($result)){
        ...
        if ($submit){
            ...
            if (!mysql_query($sql)){
                $error .= $install_lang['update_fail'].' '.$config_name.'<br />'.mysql_error();
                return $error;
            }
	...
	if ($submit){
		...
		if (!mysql_query($sql)){
			return $install_lang['update_fail'].' nuke config<br />'.mysql_error();
		}
	}
    ...
    if (!$display){
		...
		$form .= '    <dd>'.(($submit) ? (!empty($nukeurl) ? $nukeurl : 'http://'.$_SERVER['HTTP_HOST'].str_replace('/install.php', '', $_SERVER['PHP_SELF'])) : '<input type="text" name="nukeurl" size="40" class="input" value="'.(($return) ? $nukeurl : 'http://'.$_SERVER['HTTP_HOST'].str_replace('/install.php', '', $_SERVER['PHP_SELF'])).'" />').'</dd>';
		$form .= '  </dl>';
		$form .= '  <dl>';
		...
		$form .= '    <dd>'.(($submit) ? (!empty($new['sitename']) ? $new['sitename'] : 'http://'.$_SERVER['SERVER_NAME']) : '<input type="text" name="sitename" size="40" class="input" value="'.(($return) ? $new['sitename'] : 'http://'.$_SERVER['SERVER_NAME']).'" />').'</dd>';
		$form .= '  </dl>';
		$form .= '  <dl>';
		...
		$form .= '    <dd>'.(($submit) ? (!empty($new['cookie_name']) ? $new['cookie_name'] : '&nbsp;') : '<input type="text" value="'.(($return) ? $new['cookie_name'] : $_SERVER['SERVER_NAME']).'" name="cookie_name" size="40" class="input" />').'</dd>';
		$form .= '  </dl>';
		$form .= '  <dl>';
		...
		$form .= '    <dd>'.(($submit) ? (!empty($new['cookie_domain']) ? $new['cookie_domain'] : $_SERVER['HTTP_HOST']) : '<input type="text" value="'.(($return) ? $new['cookie_domain'] : $_SERVER['HTTP_HOST']).'" name="cookie_domain" size="40" class="input" />').'</dd>';
		$form .= '  </dl>';
		$form .= '  <dl>';
		...
		$form .= '    <dd>'.(($submit) ? (!empty($new['server_name']) ? $new['server_name'] : $_SERVER['SERVER_NAME']) : '<input type="text" name="server_name" value="'.(($return) ? $new['server_name'] : $_SERVER['SERVER_NAME']).'" size="40" class="input" />').'</dd>';
		$form .= '  </dl>';
		$form .= '  <dl>';
