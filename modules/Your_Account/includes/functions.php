...
function ya_userCheck($username) {
    ...
    //if ((!$username) || (empty($username)) || (preg_match("/['^À-ÿa-zA-Z0-9_-']/",$username))) $stop = "<center>"._ERRORINVNICK."</center><br />";
    //if ((!$username) || (empty($username)) || (preg_match("/['^a-zA-Z0-9['.['.'] [.'].']_{}«»=|-']/",$username))) $stop = "<center>"._ERRORINVNICK."</center><br />";
    ...
    if (isset($ya_config['bad_nick']) && !empty($ya_config['bad_nick'])) {
        ...
        if (!empty($BadNickList) && is_array($BadNickList)) {
            for ($i=0; $i < count($BadNickList); $i++) {
                if (isset($BadNickList[$i]) && !empty($BadNickList[$i]) && !empty($username)){
                    if (preg_match('/^['.$BadNickList[$i].'&#_]+$/', $username)){
...
function ya_mailCheck($user_email) {
    ...
    if (!empty($ya_config['bad_mail'])){
        $BadMailList = explode("\n",$ya_config['bad_mail']);
        for ($i=0; $i<count($BadMailList); $i++){
        	$ecount = strlen($BadMailList[$i]);
        	if (substr($user_email, -$ecount) == $BadMailList[$i]){
