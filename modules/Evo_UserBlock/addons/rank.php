   ...
   if ($userinfo['user_rank']) {
       for($j = 0, $maxj = count($ranksrow); $j < $maxj; $j++) {
         if ($userinfo['user_rank'] == $ranksrow[$j]['rank_id'] && $ranksrow[$j]['rank_special']) {
			 ...
			 $link = (preg_match('#Forums#', $link.$ranksrow[$j]['rank_image'])) ? '' : 'modules/Forums/'; 
             $rank_image = ($ranksrow[$j]['rank_image']) ? '<img src="'.$link.$ranksrow[$j]['rank_image'].'" alt="'.$poster_rank.'" title="'.$poster_rank.'" border="0" /><br />' : '';
      }
    }
   } else {
    for($j = 0, $maxj = count($ranksrow); $j < $maxj; $j++) {
